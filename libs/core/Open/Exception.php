...
class Open_Exception extends Exception
{
	...
	/**
	 * Количество строк кода рядом с вызвавшей ошибку
	 */
	const SOURCE_LINE_RANGE = 8;
	...
	{
		return preg_replace('#(\r\n|\n\r|\r|\n)#u', '<br />', htmlentities(str_replace(BASE_PATH, '', $this->getTraceAsString()), ENT_COMPAT, CHARSET));
	}

	/**
	 * Получить массив с исходным кодом участка спровоцировавшего ошибку
	 *
	 * @return string
	 */
	protected function getSafeSource()
	{
		$file = $this->getFile();
		$line = $this->getLine();

		/**
		 * Получаем исходный код участка, где произошла ошибка
		 */
		if( ($source = file($file)) !== FALSE )
		{
			/**
			 * Отсекаем лишние строки
			 */
			$temp = count($source);
			for($i = $line - self::SOURCE_LINE_RANGE - 2; $i>=0; $i--)
			{
				unset($source[$i]);
			}
			for($i = $line + self::SOURCE_LINE_RANGE; $i<$temp; $i++)
			{
				unset($source[$i]);
			}
	...
	public function handle()
	{
		...
		if( $code <= E_ALL )
		{
			...
			/**
			 * Получение исходного кода откуда вызвана ошибка
			 */
		 ...
		 */
		switch($code)
		{	case E_ERROR: case E_USER_ERROR: case E_DB: case E_403: case E_404:
				exit(0);
