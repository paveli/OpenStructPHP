...
class Open_Sync extends Open_Singleton
{
	...
	public function fileLock($key)
	{
		...
		if( !isset($this->files[$key]) )
		{
			...
			//p(strftime('%T', time()) .' '. numberFormat(fmod(microtime(true), 1), 4) ."\t $key file has been gotten");
		}
		...
		//p(strftime('%T', time()) .' '. numberFormat(fmod(microtime(true), 1), 4) ."\t $key file has been aquired");
    ...
    public function fileUnlock($key)
    {
    	...
    	if( isset($this->files[$key]) )
    	{
			//p(strftime('%T', time()) .' '. numberFormat(fmod(microtime(true), 1), 4) ."\t $key file has been released");
	...
	public function semLock($key)
	{
		...
		if( !isset($this->semaphores[$key]) )
		{
			...
			//p(strftime('%T', time()) .' '. numberFormat(fmod(microtime(true), 1), 4) ."\t $key sem has been gotten");
		}

		/**
		 * Пытаемся захватить семафор
		 */
		if( !($result = sem_acquire($this->semaphores[$key])) )
		{
			trigger_error(sprintf(Open_Text::getInstance()->dget('errors', 'Semaphore with key <b>%s</b> cannot be acquired'), $key), E_USER_WARNING);
		}

		//p(strftime('%T', time()) .' '. numberFormat(fmod(microtime(true), 1), 4) ."\t $key sem has been aquired");
    ...
    public function semUnlock($key)
    {
    	if( isset($this->semaphores[$key]) )
    	{
			//p(strftime('%T', time()) .' '. numberFormat(fmod(microtime(true), 1), 4) ."\t $key sem has been released");
